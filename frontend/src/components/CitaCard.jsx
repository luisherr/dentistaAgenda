...
import { Check, X, UserCheck, Clock, Undo2, Pencil, AlertTriangle } from 'lucide-react';
...
export default function CitaCard({ cita, onConfirmar, onCancelar, onAtender, onRevertir, onEditar }) {
  ...
  return (
    <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-200 dark:border-dark-border shadow-sm overflow-hidden">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
          ...
          {cita.estado === 0 && (
            <>
              ...
              <Button variant="danger" size="sm" onClick={() => setConfirmandoCancelar(true)} title="Cancelar">
                <X className="w-4 h-4" />
              </Button>
            </>
          )}
      ...
      {confirmandoCancelar && (
        <div className="px-4 py-3 bg-red-50 dark:bg-red-900/20 border-t border-red-200 dark:border-red-800 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <AlertTriangle className="w-4 h-4 text-red-500 flex-shrink-0" />
            <p className="text-sm text-red-700 dark:text-red-300 truncate">Cancelar cita de {cita.nombrePaciente}?</p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <Button variant="outline" size="sm" onClick={() => setConfirmandoCancelar(false)}>
              No
            </Button>
